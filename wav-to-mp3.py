...
"""Converts all .wav files inside current directory to .mp3 using ffmpeg."""
...
def convert(wav_path, mp3_path, compression_level, delete_original):
    """Convert one file. Returns bytes saved on success, an error-message
    string on failure, None if skipped."""
    if mp3_path.exists():
        with print_lock:
            print(f"Skipped: {wav_path.name} (output already exists: {mp3_path.name})", file=sys.stderr)
        return None
    ...
    try:
        ...
        result = subprocess.run(
            [
                ...
                "-f", "mp3",  # the .part suffix hides the format from ffmpeg
                str(part_path),
            ],
            capture_output=True,
            text=True,
    ...
    with print_lock:
        print(f"Converted: {wav_path.name} -> {mp3_path.name}{deleted_note}")
    return wav_size - mp3_size
...
def compression_level_arg(value):
    try:
        ...
        raise argparse.ArgumentTypeError(f"invalid compression level: {value!r} (must be an integer 0-9)")
    if not 0 <= level <= 9:
        raise argparse.ArgumentTypeError(f"compression level must be between 0 and 9, got {level}")
    return level
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Converts all .wav files inside current directory to .mp3 using ffmpeg.",
    ...
    # write each .mp3 in place next to its source so it replaces the original.
    if delete_original:
        mp3_paths = [p.with_suffix(".mp3") for p in wav_files]
    else:
        mp3_paths = [converted_root / p.relative_to(cwd).with_suffix(".mp3") for p in wav_files]
    ...
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        futures = {
            ...
            for wav_path, mp3_path in zip(wav_files, mp3_paths)
        }
